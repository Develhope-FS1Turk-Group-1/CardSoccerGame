	...
	try {
		...
		bcrypt.hash(password, 10, async (err, hash) => {
			if (err) {
				...
				try {
					const checkUserIfExist = await pool.query(queryCheck, [
						mail,
						username,
					]);
					if (checkUserIfExist.rows.length > 0) {
						return res.status(409).json({
							message:
								'User registered before please control your information!!!',
						});
					}
				} catch (error) {
	...
	try {
		...
		if (result.rows.length === 1) {
			...
			if (user.is_activated) {
				...
				res.status(401).json({
					message:
						'You have not been activated your account. Please activate it first',
				});
			}
		} else {
			res.status(401).json({ message: 'Invalid Credentials' });
