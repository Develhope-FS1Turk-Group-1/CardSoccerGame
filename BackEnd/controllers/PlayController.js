		...
		pool.query(bestMidfieldersQuery, [team], (err, midfielders) => {
			...
			pool.query(bestDefendersQuery, [team], (err, defenders) => {
				...
				pool.query(bestGoalkeeperQuery, [team], async (err, goalkeeper) => {
					...
					const userTeamPower = await calculatedAllPower.teamPower;
					const userGoal = Math.floor((Math.random() * userTeamPower) / 150);
					const opponentGoal = Math.floor((Math.random() * teamPowerValue) / 150);
...
const addMatchHistory = async (req, res) => {
	...
	if (userId === undefined || opponentId === undefined || userGoal === undefined || opponentGoal === undefined) {
    ...
    `;
		...
		const values = [userId, opponentId, opponentType, userGoal, opponentGoal];
...
const getMatchHistoryById = async (req, res) => {
  ...
  const limit = displayCount && displayCount > 5 ? displayCount : 5;
